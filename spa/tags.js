...
riot.tag2('fixture', '<h2>Current Point Table</h2> <table class="table-bordered table table-hover table-striped"> <tr> <th>Team</th> <th>GP</th> <th>Pts</th> <th>W</th> <th>D</th> <th>L</th> <th>GF</th> <th>GA</th> <th>+/-</th> </tr> <tr each="{_data}"> <td>{team}</td> <td>{games_played}</td> <td>{point}</td> <td>{wins}</td> <td>{draw}</td> <td>{lost}</td> <td>{goals_forward}</td> <td>{goals_against}</td> <td>{goal_difference}</td> </tr> </table> <div class="row"> <div class="col-md-12"><a class="pull-right btn btn-warning" onclick="{finish}">Play All</a>&nbsp;<a class="pull-right btn btn-danger" onclick="{reset}">Reset Fixture</a></div> </div>', '', '', function(opts) {
    _data = [];
    var self = this;
...
riot.tag2('thisweek', '<h3>{_data.season.week}. Week Matches</h3> <table class="table-striped table table-hover table-bordered"> <tbody> <tr class="d-flex" each="{_data.matches}"> <td class="col-md-5">{home.team}</td> <td class="col-md-1">{score_home}</td> <td class="col-md-1">{score_away}</td> <td class="col-md-5">{away.team}</td> </tr> </tbody> </table><a class="btn btn-success" onclick="{play}">Play Week</a>', '', '', function(opts) {
    var self = this;
    _data = {
        matches: [],
        season: {}
